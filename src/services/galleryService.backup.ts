...
export class GalleryService {
  ...
  static async getGeneratedImages(filters?: GalleryFilters): Promise<GeneratedImage[]> {
    ...
    if (!this.isSupabaseAvailable()) {
      ...
      console.log('Debug: Total demo images in storage:', this.demoGeneratedImages.size);
      console.log('Debug: Filters applied:', filters);
      
      let images = Array.from(this.demoGeneratedImages.values());
      console.log('Debug: Images before filtering:', images.length);
      
      // Apply filters
      if (filters?.country) {
        const country = await this.getCountryByISO(filters.country);
        console.log('Debug: Country filter lookup:', { iso: filters.country, country });
        if (country) {
          const beforeFilter = images.length;
          images = images.filter(img => img.country_id === country.id);
          console.log('Debug: After country filter:', { beforeFilter, afterFilter: images.length, countryId: country.id });
        }
      }
      ...
      if (filters?.role) {
        ...
        console.log('Debug: After role filter:', { beforeFilter, afterFilter: images.length, role: filters.role });
      }
      ...
      console.log('Debug: Final filtered images:', {
        total: images.length,
        filters,
        images: images.map(img => ({ id: img.id, style_id: img.style_id, role: img.role, country_id: img.country_id }))
      });
