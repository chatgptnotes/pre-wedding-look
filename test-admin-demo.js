...
class AdminDemoTester {
  ...
  async testCountryModelsManager() {
    ...
    try {
      ...
      const countryModelsButton = await this.page.$('text="Country Models Manager"');
      if (!countryModelsButton) {
        // Try alternative selector
        await this.page.click('button:has-text("Country Models Manager")').catch(() => {
          // Find and click the button containing the earth emoji and text
      ...
      const statsSection = await this.page.$('text="Gallery Statistics"');
      if (statsSection) {
  ...
  async testStyleApplicationSystem() {
    ...
    try {
      ...
      const batchSection = await this.page.$('text="Batch Operations"');
      if (batchSection) {
      ...
      const styleCards = await this.page.$$('[class*="grid"], [class*="card"]');
      ...
      const queueStatus = await this.page.$$('text="Processing", text="Pending", text="Queued"');
      if (queueStatus.length > 0) {
        this.results.styleApplication.details.push('✅ Queue status indicators present');
      } else {
        this.results.styleApplication.details.push('ℹ️ No queue status indicators (expected in demo mode)');
      }
  ...
  async testGalleryFeatures() {
    ...
    try {
      ...
      await this.page.click('text="Back to Creative Studio"').catch(() => {
      ...
      const galleryTab = await this.page.$('text="Gallery"');
      if (galleryTab) {
