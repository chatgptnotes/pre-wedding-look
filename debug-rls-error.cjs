...
async function runDiagnostics() {
  ...
  // Test 7: Check if table exists and has correct structure
  console.log('\n🗂️ TEST 7: Table Structure Check');
  console.log('=================================');
  ...
  if (SUPABASE_SERVICE_KEY) {
    ...
    try {
      console.log('Checking RLS status on country_models...');
      const { data, error } = await serviceClient.rpc('exec_raw_sql', {
        query: `
          SELECT 
            schemaname,
            tablename,
      ...
      if (error) {
        console.log('⚠️  Cannot check RLS status (this is expected):', error.message);
      } else {
        console.log('✅ RLS Status:', data);
      }
    } catch (err) {
      console.log('⚠️  RLS status check failed (this is expected):', err.message);
    }
  }
